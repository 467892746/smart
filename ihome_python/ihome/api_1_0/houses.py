...
def get_house_list():
    ...
    try:
        if start_date and end_date:
            # 查询冲突的订单
            confict_orders = Order.query.fliter(Order.begin_date <= end_date, Order.end_date >= end_date)
        elif start_date:
            confict_orders = Order.query.fliter(Order.end_date >= start_date)
        elif end_date:
    ...
    if confict_orders:
        # 从订单中获取冲突的房屋id
        confict_house_ids = [order.house.id for order in confict_orders]
    ...
    # 查询数据库
    # 补充排序条件
    if sort_key == 'booking': # 入住
        house_query = House.query.filter(*filter_params).order_by(House.order_count.desc())
    elif sort_key == 'price-inc':
        house_query = House.query.filter(*filter_params).order_by(House.price.arc())
