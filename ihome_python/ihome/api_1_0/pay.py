...
def save_order_payment_result():
    ...
    # 对支付包的数据进行分离 提取出支付宝的签名参数sing和剩下的其他数据
    ...
    if result:
        ...
        trade_no = alipay_dict.get('trade_no') # 支付宝交易号
        try:
            Order.query.fliter_by(id=order_id).update({'status':"WAIT_COMMENT", 'trade_no':trade_no})
            db.session.commit()
        except Exception as e:
            current_app.logger.error(e)
            db.session.rollback()
